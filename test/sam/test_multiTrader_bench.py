...
# 写作日期: 20180708
#=========================================================
import sys,unittest,json,requests,time,grpc,datetime,threading,yaml
...
accountList = yaml.load(open('fm_account.yml', 'r',encoding='utf-8'))

# for i in accountList["fm_account"]:
#     print(i)
...
class MaxAccountPlsitionLots(unittest.TestCase):
    ...
    def test_maxPositionLots_tradeEqual(self):
        ...
        def myThread(account):
            #交易员登陆---------------------
            tradeDatas = {"account": account, "password":webAPIData['passwd'], "remember":"false"}
            ...
            #获取指定经纪商的accountIndex。当前为：pcio
            self.tradePicoAccountIndex = Account.getSpecialAccountIndex(headers = self.tradeHeaders,accountType=2, brokerID=106)[0]
            ...
            print(datetime.datetime.now(),i,self.tradeMt4Account,self.tradeBrokerID)
        ...
        for account in accountList["fm_account"]:
            # 启动N个线程
            for i in range(1):
