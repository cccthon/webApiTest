...
class SamPosition(unittest.TestCase):
    ...
    def test_SamPosition(self):
        ...
        #部分平仓
        partClose = TradeSAM.TradeSAMStream.ClosePosition(stub=self.stub, account=self.account, brokerID=self.brokerID, tradeID=self.tradeID, lots=self.partLots)
        comment = partClose.Signal.Comment
        self.newTradeID = comment.split("#")[-1]
        self.assertEqual(partClose.Signal.TradeID, self.tradeID)
        self.assertEqual(partClose.Signal.Symbol, self.symbol)
        self.assertEqual(partClose.Signal.Lots, self.partLots)

    def tearDown(self):
        # 退出登录
        # 平仓剩余手数
        closeRes = TradeSAM.TradeSAMStream.ClosePosition(stub=self.stub, account=self.account, brokerID=self.brokerID, tradeID=int(self.newTradeID), lots=0.2)
        self.assertEqual(closeRes.Signal.TradeID, int(self.newTradeID))
