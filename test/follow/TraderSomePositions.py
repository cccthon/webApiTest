...
class TraderSomePositions(unittest.TestCase):
    def setUp(self):
        #跟随者登录
    ...
    def test_TraderSomePositions(self):
        '''登录->建立跟随->跟随下单->跟随部分平仓->查看跟随交易历史订单->取消跟随'''
        ...
        # 获取跟随者的accountindex
        self.followerPicoAccountIndex = Account.getSpecialAccountIndex(headers=userData['headers'], brokerID=5)

        # 新建一个跟随,固定手数跟随
        params = {"accountIndex": int(self.followerPicoAccountIndex[0]), "strategy": "fixed", "setting": 1.5,
                  "direction": "positive"}
        ...
        Account.switchAccount(userData['hostName'] + userDataAccount['switchAccount'], userData['headers'],
                              self.followerPicoAccountIndex[0])
        ...
        cancelFollowRes = Follow.DeleteFollow(
            userData['hostName'] + userDataFollow["Follow_Url"] + str(self.traderUserId) + "_" +
            self.tradeAccountIndex[0] + "?accountIndex=" + self.followerPicoAccountIndex[0],
            userData['headers'])
...
if __name__ == '__main__':
    suite = unittest.TestSuite(unittest.makeSuite(TraderSomePositions))
