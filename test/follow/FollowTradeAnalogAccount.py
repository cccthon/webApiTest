...
class FollowTradeAnalogAccount(unittest.TestCase):
    def setUp(self):
        #通过邮箱注册账号，获取模拟账户
    ...
    def test_FollowTradeAnalogAccount(self):
        '''登录->(模拟账户)建立跟随(固定)->跟随下单->跟随平仓->查看跟随交易历史订单->取消跟随'''
        ...
        params = {"accountIndex": int(self.followerAccountIndex[0]),
                  "strategy": "fixed", "setting": 0.1, "direction": "positive"}
        ...
        DEMOMT4Account = str(json.loads(demotoken.content)["data"]["MT4Account"])

        #  验证demo跟随者跟单成功
        sql = "SELECT TradeID from t_followorder where Account=" + DEMOMT4Account + " and TraderTradeID=" + str(
            self.orderID)
        row = FollowOperation.Operation.operationCopytradingDB(sql)

        # 查询跟随者的历史订单
        followerHistoryOrdersRes = Trade.getOrders(userData['hostName'] + userDataTrade["getOrders"], userData['headers'],
                                             userData['orderStatus_close'])
...
if __name__ == '__main__':
    suite = unittest.TestSuite(unittest.makeSuite(FollowTradeAnalogAccount))
