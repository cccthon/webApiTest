...
# 用例标题: 获取交易员个人展示页2.1交易概览数据对比MongoDB数据
# 预置条件: 
# 测试步骤:
#   1.获取个人展示页2.1交易概览数据对比MongoDB数据
# 预期结果:
#   1.检查响应码为：200
# 脚本作者: zhangyanyun
# 写作日期: 20180719
...
class StatisticsOfAccount(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        table_3 = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","平均获利","平均亏损","最大盈利金额","最大亏损金额"])
        table_4 = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","日均交易手数","隔夜利息","点差","日均交易笔数","交易胜率"])
        ...
        try:
            ...
            table_2.add_row(["","","","","","","","",""])
            table_3.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,self.mongoList["money_profit_close_avg"],self.mongoList["money_loss_close_avg"],self.mongoList["money_profit_close_max"],self.mongoList["money_loss_close_min"]])
            table_3.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,self.item["AvgProfitMoney"],self.item["AvgLossMoney"],self.item["MaxProfitMoney"],self.item["MaxLossMoney"]])
            table_3.add_row(["","","","","","","","","",""])
            table_4.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,self.mongoList["standardlots_close"]/self.mongoList["period_trade"]/5],self.mongoList["swaps"],self.mongoList["commission"],self.mongolist["deal_amount_per_day"],self.mongoList["rate_profit"])
            table_4.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,self.item["AvgDayStandardLots"]],self.item["Swaps"],self.item["Commission"],item["DealAmountPerDay"],item["RateProfit"])
            table_4.add_row(["","","","","","",""])
    ...
    def test_16_FactorProfitEquity(self):
        #日均交易手数
    ...
    def test_17_SellProfitOrders(self):
        #隔夜利息
        self.assertAlmostEqual(self.mongoList["swaps"],float(self.item["Swaps"]),delta = 0.01)

    def test_18_SellProfitOrders(self):
        #点差
        self.assertAlmostEqual(self.mongoList["commission"],float(self.item["Commission"]),delta = 0.01)

    def test_19_SellProfitOrders(self):
        #日均交易笔数
        self.assertAlmostEqual(self.mongoList["deal_amount_per_day"],float(self.item["DealAmountPerDay"]),delta = 0.01)

    def test_20_SellProfitOrders(self):
        #交易胜率
        self.assertAlmostEqual(self.mongoList["rate_profit"],float(self.item["RateProfit"]),delta = 0.01)
