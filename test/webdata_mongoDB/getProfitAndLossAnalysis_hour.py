#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_getProfitAndLossAnalysis_hour

# 用例标题: 获取盈亏分析图表(小时)数据对比MongoDB数据
# 预置条件: 
# 测试步骤:
# 1.获取盈亏分析图表(小时)数据对比MongoDB数据
# 预期结果:
#   1.检查响应码为：200
# 脚本作者: zhangyanyun
# 写作日期: 20180521
...
class HotTrader(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # profitAndLossAnalysis_timeTypeHour = Datastatistic.getProfitAndLossAnalysis("http://dev.fmfe.com/api/v2/trade/77141_1/month/analysis/report")
        self.profitAndLossAnalysis = Datastatistic.getProfitAndLossAnalysis(webAPIData['hostName'] + datastatisticData['getProfitAndLossAnalysis_url'] + str(self.userID) + "_"+ str(self.AccountIndex) + datastatisticData["getProfitAndLossAnalysis_url2"],params="TimeTypeHour",printLogs=0)
        ...
        for self.item in json.loads(self.profitAndLossAnalysis.text)["data"]["HourAndWeekList"]:
            #遍历返回数据的所有小时
            self.hourList.append(self.item["Hour"])
            #遍历返回数据的所有收益
            ...
            # for i in mongoDB.fm.mg_result_hour.find({"_id":str(self.mt4Account) + "_" + str(self.brokerid) + str(otherStyleMonth) + str(self.item["Hour"])}):
            for i in mongoDB.datastatistic.mg_result_hour.find({"login":str(self.mt4Account),"close_hour":self.item["Hour"]}):
            # i = mongoDB.fm.mg_result_hour.find({"login":str(self.mt4Account),"close_hour":self.item["Hour"]})
                self.mongoList = {}
            ...
            table_1 = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","小时","平仓收益","平仓做多盈利收益","平仓做多亏损收益","平仓做空盈利收益","平仓做空亏损收益","做多手数"])
            table_2 = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","小时","平仓点数","平仓做多盈利点数","平仓做多亏损点数","平仓做空盈利点数","平仓做空亏损点数","做空手数"])

            try: 
                table_1.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,self.item["Hour"],self.mongoList["money_close"],self.mongoList["money_profit_long_close"],self.mongoList["money_loss_long_close"],self.mongoList["money_profit_short_close"],self.mongoList["money_loss_short_close"],self.mongoList["standardlots_long_close"]])
                table_1.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,self.item["Hour"],self.item["Money"],self.item["MoneyProfitLong"],self.item["MoneyLossLong"],self.item["MoneyProfitShort"],self.item["MoneyLossShort"],self.item["StandardLotsLong"]])
                table_1.add_row(["","","","","","","","","","",""])
                table_2.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,self.item["Hour"],self.mongoList["point_close"],self.mongoList["point_profit_long_close"],self.mongoList["point_loss_long_close"],self.mongoList["point_profit_short_close"],self.mongoList["point_loss_short_close"],self.mongoList["standardlots_short_close"]])
                table_2.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,self.item["Hour"],self.item["Pips"],self.item["PipsProfitLong"],self.item["PipsLossLong"],self.item["PipsProfitShort"],self.item["PipsLossShort"],self.item["StandardLotsShort"]])
                table_2.add_row(["","","","","","","","","","",""])
