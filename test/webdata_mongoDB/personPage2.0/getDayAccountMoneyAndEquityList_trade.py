...
class DayAccountMoneyAndEquityList(unittest.TestCase):
    def setUp(self):
        ...
        for self.item in json.loads(MoneyAndEquity.text)["data"]["MoneyList"]:
            ...
            table = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","日期","已平仓收益"])

            try:
                table.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,otherStyleTime,self.mongoList_day["money_close"]])
                table.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,otherStyleTime,self.item["Money"]])
                table.add_row(["","","","","",""])
        ...
        for self.items in json.loads(MoneyAndEquity.text)["data"]["MonthMoneyList"]:
            ...
            self.monthmoneyList.append(self.items["Money"])
            ...
            table = PrettyTable(["预期/实际","UserID","AccountIndex","MT4Account","月份","当月收益"])

            try:
                table.add_row(["预期结果",self.userID,self.AccountIndex,self.mt4Account,otherStyleMonth,self.mongoList_month["money_close"]])
                table.add_row(["实际结果",self.userID,self.AccountIndex,self.mt4Account,otherStyleMonth,self.items["Money"]])
                table.add_row(["","","","","",""])
    ...
    def test_2_Monthmoney(self): 
        #当月收益
        self.assertAlmostEqual(self.mongoList_month["money_close"],float(self.items["Money"]),delta = 0.01)
