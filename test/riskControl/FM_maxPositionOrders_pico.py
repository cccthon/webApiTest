...
class MaxPositionLots(unittest.TestCase):
    def setUp(self):
        #交易员登陆---------------------
        ...
        #设置针对单个交易员的风控参数。最大持仓单数为2单
        specialTradeDatas = { "accountIndex": self.followPicoAccountIndex, webAPIData['maxPositionOrders']: 2}
    ...
    def test_maxPositionLots_tradeOnceMoreOnce(self):
        '''持仓总单数设置为2，交易员买2单，平掉一单后继续买一单。检查跟随者跟随订单情况'''
        #下单时的手数为实际值除以100
        openParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_210'], tradeOnlineData['orderParam_symbol']: tradeOnlineData['broker_EURCAD'], tradeOnlineData['orderParam_volume']: 300 }
        TradeOnline.OnlineTradeEvent.tradeEvent(self, tradeOnlineData['ws_host'], tradeOnlineData['ws_port'], {'token': self.tradeToken}, openParam)
        openPositionRes = TradeOnline.OnlineTradeEvent.tradeEvent(self, tradeOnlineData['ws_host'], tradeOnlineData['ws_port'], {'token': self.tradeToken}, openParam)
        ...
        #继续下一单
        openParam1 = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_210'], tradeOnlineData['orderParam_symbol']: tradeOnlineData['broker_EURCAD'], tradeOnlineData['orderParam_volume']: 300 }
        ...
        #校验手数为下单时的手数为实际值除以100(300/100)
        self.assertEqual(openPositionRes1["order"]["volume"], 300)
        #保存orderID,待开仓使用
