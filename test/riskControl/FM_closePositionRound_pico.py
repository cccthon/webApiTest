...
#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_closePositionRound_pcio
# 用例标题: 风控全局平仓小数点取舍
# 预置条件: 
# 测试步骤:
#   1.
...
class ClosePositionRound(unittest.TestCase):
    def setUp(self):
        #交易员登陆---------------------
        ...
        print("self.tradePicoAccountIndex",self.tradePicoAccountIndex)
        # self.assertEqual(self.tradePicoAccountIndex, webAPIData['status_code_200'])
        ...
        webAPIData['follow_setting']: 0.1, webAPIData['followDirection']: webAPIData['follow_positive']}
    ...
    def test_closePositionRound_floor(self):
        '''设置平仓手数小数位取小。交易员下单15.5手。检查跟随者跟随1.5手。交易员平掉5手，跟随者应该跟随部分平掉：0.4手，剩余1.1手'''
        #设置全局风控参数。设置平仓小数点取舍为：取小
        globalRiskControlData = {webAPIData['closePositionRound']: webAPIData['Floor']}
        ...
        openParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_210'], tradeOnlineData['orderParam_symbol']: tradeOnlineData['broker_EURCAD'], tradeOnlineData['orderParam_volume']: 1550 }
        ...
        #断言订单列表不为空
        self.assertNotEqual(json.loads(getFollowOrder.text)['data']['items'], [])
        #断言跟随者订单跟随的品种为：EURCAD
        self.assertEqual(json.loads(getFollowOrder.text)['data']['items'][0]['SYMBOL'], tradeOnlineData['broker_EURCAD'])
        #断言跟随者订单跟随的手数为：0.1手
        self.assertEqual(float(json.loads(getFollowOrder.text)['data']['items'][0]['VOLUME']), 1.55)

        #交易员平掉测试订单5手
        partCloseParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_211'], tradeOnlineData['orderParam_ticket']: self.orderID, tradeOnlineData['orderParam_volume']: 500 }
        partClosePosition = TradeOnline.OnlineTradeEvent.tradeEvent(self, tradeOnlineData['ws_host'], tradeOnlineData['ws_port'], {'token': self.tradeToken}, partCloseParam)
        #校验code等于0，为及时单平仓成功
        self.assertEqual(partClosePosition["code"], tradeOnlineData['ws_code_0'])
        #校验rcmd等于210，为及时单开仓
        ...
        #部分平仓生成的新订单号
        self.newOrderID = int(partClosePosition["order"]["comment"].split('#')[-1])
        ...
        self.assertEqual(json.loads(getFollowOrder.text)['data']['items'][0]['SYMBOL'], tradeOnlineData['broker_EURCAD'])
        #断言跟随者订单跟随的手数为：0.1手
        self.assertEqual(float(json.loads(getFollowOrder.text)['data']['items'][0]['VOLUME']), 1.05)

        #平掉测试订单
        closeParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_211'], tradeOnlineData['orderParam_ticket']: self.newOrderID, tradeOnlineData['orderParam_volume']: 1050 }
    ...
    def test_closePositionRound_Ceil(self):
        ...
        globalRiskControlData = {webAPIData['closePositionRound']: webAPIData['Ceil']}
        setRiskControl = RiskControl.setRiskControl(webAPIData['hostName'] + riskControlData['setRiskControl_url'], accountIndex=self.followPicoAccountIndex, headers = self.followHeaders, datas=globalRiskControlData)
        self.assertEqual(setRiskControl.status_code, webAPIData['status_code_200'])
