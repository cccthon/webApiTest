...
#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_openPositionRound_pcio
# 用例标题: 风控全局开仓小数点取舍
# 预置条件: 
# 测试步骤:
#   1.
...
class OpenPositionRound(unittest.TestCase):
    def setUp(self):
        #交易员登陆---------------------
    ...
    def test_openPositionRound_floor(self):
        '''设置开仓手数小数位取小。交易员下单1.5手。检查跟随者跟随0.1手'''
        #设置全局风控参数。设置开仓小数点取舍为：取小
        globalRiskControlData = {webAPIData['openPositionRound']: webAPIData['Floor']}
        ...
        openParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_210'], tradeOnlineData['orderParam_symbol']: tradeOnlineData['broker_EURCAD'], tradeOnlineData['orderParam_volume']: 150 }
        ...
        self.assertEqual(openPositionRes["order"]["volume"], 150)
        #保存orderID,待开仓使用
        ...
        #断言跟随者订单跟随的手数为：0.1手
        self.assertEqual(float(json.loads(getFollowOrder.text)['data']['items'][0]['VOLUME']), 0.15)

        #平掉测试订单
        closeParam = { tradeOnlineData['orderParam_code']: tradeOnlineData['ws_code_211'], tradeOnlineData['orderParam_ticket']: self.orderID, tradeOnlineData['orderParam_volume']: 150 }
    ...
    def test_openPositionRound_ceil(self):
        '''设置开仓手数小数位取大。交易员下单1.5手。检查跟随者跟随0.2手'''
        #设置全局风控参数。设置开仓小数点取舍为：取小
        globalRiskControlData = {webAPIData['openPositionRound']: webAPIData['Ceil']}
        setRiskControl = RiskControl.setRiskControl(webAPIData['hostName'] + riskControlData['setRiskControl_url'], accountIndex=self.followPicoAccountIndex, headers = self.followHeaders, datas=globalRiskControlData)
        self.assertEqual(setRiskControl.status_code, webAPIData['status_code_200'])
