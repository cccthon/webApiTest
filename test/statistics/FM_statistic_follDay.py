...
class UserInfo(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        # brokerID = 5
        # #查询对某个交易员的跟随数据
        # masteraccount = "200028927"
        # masterbrokerid = 1
        # close_date = "2017-11-14"

        # #最近存在交易数据帐号
        login = '830012'
        brokerID = 5
        #查询对某个交易员的跟随数据
        masteraccount = "830005"
        masterbrokerid = 5
        close_date = "2018-06-12"

        table = 'mg_result_follday'
        mongoFind = {"login":login,"masteraccount":masteraccount,"masterbrokerid":masterbrokerid,"close_date":close_date}
        ...
        self.prestoList = Statistic.prestoData(login = login, brokerID = brokerID,day=close_date,masteraccount=masteraccount)
    ...
    def test_4_money_profit_close(self):
        """盈利订单收益"""
        self.assertAlmostEqual(sum(self.prestoList.money_profit_sum), self.mongoList.money_profit_close,delta=0.1)
