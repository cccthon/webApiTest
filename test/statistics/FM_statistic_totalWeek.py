...
class UserInfo(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        #查询星期几的数据
        week = 2
        mongoFind = {"login": login,"close_week": week}
        ...
        self.prestoList = Statistic.prestoData(login = login, brokerID = brokerID,week=week)
    ...
    def test_2_money_profit_short_close(self):
        """做空盈利订单收益"""
        self.assertAlmostEqual(sum(self.prestoList.money_profit_short_close), self.mongoList.money_profit_short_close,delta=0.1)

    def test_3_money_loss_short_close(self):
        """做空亏损订单收益"""
        self.assertAlmostEqual(sum(self.prestoList.money_loss_short_close), self.mongoList.money_loss_short_close,delta=0.1)
    ...
    def test_7_point_profit_short_close(self):
        """做空盈利订单点数"""
        self.assertAlmostEqual(sum(self.prestoList.point_profit_short_close), self.mongoList.point_profit_short_close,delta=0.8)

    def test_8_point_loss_short_close(self):
        """做空亏损订单点数"""
        self.assertAlmostEqual(sum(self.prestoList.point_loss_short_close), self.mongoList.point_loss_short_close,delta=0.1)
